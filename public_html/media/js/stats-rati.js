	...
	$('select[name=\'theme\']').each(function() {
		...
		$(this).change(function() {
			...
			if ($(this).find('option:selected').text() == 'Custom') {
				...
				target.data('interval', window.setInterval(function() { updatePreview(target); }, 750));
				target.animate({width: '54em'}, function() {
					target.find('.custom-theme').show();
				});
			} else {
				target.find('.custom-theme').hide();
				target.animate({width: '35em'});
